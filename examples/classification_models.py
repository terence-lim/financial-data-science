...
show(summary, caption='Stratified Train/Test Split by Event', **SHOW)
...
show(DataFrame([[x_train.shape, x_test.shape]],
               index=['data shape:'],
               columns=['train', 'test']),
     caption="n_sample x n_features", **SHOW)
...
for name, clf in tqdm(models.items()):
    ...
    show(DataFrame.from_dict(results, orient='index'),
         caption="Train and Test Accuracy", **SHOW)
...
show(DataFrame.from_dict({'Tree depth': dt.get_depth(),
                         ...
                         columns=['property'],
                         orient='index'),
     caption="Fitted Decision Tree", **SHOW)
"""
Tree depth: 431
Tree n_leaves: 59217

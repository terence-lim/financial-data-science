...
ALL = True
outdir = '/home/terence/Downloads/out/'
html = 'index.html'

if False: # ALL Construct Momentum and Price signals: mom12m, mom1m, mom36m, mom6m, chmom, indmom, divyld
    beg, end = 19300101, 20190630
    ...
    for pordate in bd.endmo_range(bd.endmo(beg, 12), end):
        ...
        df = df.join(crsp.get_section('names',['siccd','exchcd','shrcd'],'date',pordate,0), how='left')
...
if False: #ALL:  # Construct liquidity signals from daily returns
...
if False: #ALL: # Construct weekly-returns based signals
...
if False: #ALL: # Construct fundamental signals from PSTAT Annual:
...
if False: #ALL: # Construct fundamental signals from PSTAT Quarterly
...
if False:   # Construct IBES FY1 (FPI=1) signals: chfeps, chnanalyst, disp, nanalyst
...
if False:   # Construct IBES LTG (FPI=0) signals: fgr5yr
    columns = ['fgr5yr']
...
if False: #  Construct earnings announcement date (rdq) in PSTAT linked to CRSP daily: ear, aevol
    columns = ['ear', 'aeavol']
...
if False:  #  Construct IBES FY1 signals linked to Quarterly PSTAT: sfe
...
if ALL:   # Construct IBES Q1 (FPI='6') signals linked to Quarterly PSTAT: sue
    columns = ['sue']
    numlag = 4
    end = 20190228
